...
{
    public long GetSeedWithClosesLocation(string[] lines)
    {
        var seeds = GetSeeds(lines[0]);
        var maps = GetMaps(lines[2..]);

        var finalLocations = seeds.Select(s => GetFinalSeedLocation(s, maps));
    ...
    private static long GetNextDestination(long seed, SeedMap map)
    {
        ...
        {
            if (configuration.Source <= seed 
                && seed <= configuration.Source + configuration.Range)
            {
